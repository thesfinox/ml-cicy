...
# FUNCTIONAL_MATRIX:
#
#   Compute a Functional model for the data using the entire matrix.
#
...
def compute(df_name, rounding=np.floor, seed=42):
    ...
    # Print banner
    print('\n----- KERAS FUNCTIONAL MODEL -----', flush=True)
    ...
    # Plot the model
    model_cnn_plot = path.join(IMG_PATH, 'cnn_functional.png')
    ...
    # Show a summary
    print('\nFunctional model with the entire matrix...', flush=True)
    model_cnn.summary()
    ...
    if path.isfile(path.join(MOD_PATH, 'cnn_functional.h5')):
        model_cnn = load_model(path.join(MOD_PATH, 'cnn_functional.h5'))
    else:
        print('\nCannot load the best model!', flush=True)
        
    print('    Accuracy (rint) on the training set for h_11: {:.3f}%'.format(\
                    accuracy_score(h11_labels_nn_train,
                                   model_cnn.predict([num_cp_input_train,
                                                      dim_cp_input_train_conv,
                                                      dim_h0_amb_input_train_conv,
                                                      df_matrix_nn_train])[0].\
                                                              reshape(-1,),
    ...
    save_fig('cnn_functional_error_eng')
    # plt.show()
    plt.close(fig)
