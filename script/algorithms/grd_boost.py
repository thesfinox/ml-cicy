...
def compute(df_name, n_iter=30, rounding=np.floor, seed=42):
    ...
    search_params = {'loss':              Categorical(['ls', 'lad', 'huber']),
                     'learning_rate':     Real(1e-4, 1e1,
                                               base=10,
                                               prior='log-uniform'),
    ...
    prediction_score(grd_boost_h11,
                     df_matrix_test,
                     h11_labels_test,
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          grd_boost_h11.best_estimator_.predict(df_matrix_test),
                          rounding=rounding),
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          grd_boost_h11.best_estimator_.predict(df_num_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{11}$')
    count_plot(plot,
               error_diff(h21_labels_test,
                          grd_boost_h21.best_estimator_.predict(df_num_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('grd_boost_error_num_cp')
    # plt.show()
    plt.close(fig)
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          grd_boost_h11.best_estimator_.predict(df_dim_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{11}$')
    count_plot(plot,
               error_diff(h21_labels_test,
                          grd_boost_h21.best_estimator_.predict(df_dim_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('grd_boost_error_dim_cp')
    # plt.show()
    plt.close(fig)
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          grd_boost_h11.best_estimator_.predict(df_eng_h11_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{11}$')
    count_plot(plot,
               error_diff(h21_labels_test,
                          grd_boost_h21.best_estimator_.predict(df_eng_h21_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('grd_boost_error_eng')
    # plt.show()
    plt.close(fig)
