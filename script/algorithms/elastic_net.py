...
# EL_NET:
#
#   Compute the ElasticNet model for the data.
#
...
def compute(df_name, n_iter=30, rounding=np.floor, seed=42):
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          el_net_h11.best_estimator_.predict(df_matrix_test),
                          rounding=rounding),
    ...
    count_plot(plot,
               error_diff(h21_labels_test,
                          el_net_h21.best_estimator_.predict(df_matrix_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('el_net_error_matrix')
    # plt.show()
    plt.close(fig)
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          el_net_h11.best_estimator_.predict(df_num_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{11}$')
    count_plot(plot,
               error_diff(h21_labels_test,
                          el_net_h21.best_estimator_.predict(df_num_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('el_net_error_num_cp')
    # plt.show()
    plt.close(fig)
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          el_net_h11.best_estimator_.predict(df_dim_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{11}$')
    count_plot(plot,
               error_diff(h21_labels_test,
                          el_net_h21.best_estimator_.predict(df_dim_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('el_net_error_dim_cp')
    # plt.show()
    plt.close(fig)
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          el_net_h11.best_estimator_.predict(df_eng_h11_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{11}$')
    count_plot(plot,
               error_diff(h21_labels_test,
                          el_net_h21.best_estimator_.predict(df_eng_h21_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('el_net_error_eng')
    # plt.show()
    plt.close(fig)
