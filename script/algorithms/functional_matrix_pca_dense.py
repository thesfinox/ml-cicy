...
#
#   Compute a Functional model for the data using the PCA of the matrix but
...
def compute(df_name, rounding=np.floor, seed=42):
    ...
    # Plot the model
    model_cnn_plot = path.join(IMG_PATH, 'cnn_functional_pca_dense.png')
    ...
    model_cnn_history = model_cnn.fit(x=[num_cp_input_train,
                                         dim_cp_input_train,
                                         dim_h0_amb_input_train,
                                         matrix_pca_input_train],
                                      ...
                                      validation_data=([num_cp_input_val,
                                                        dim_cp_input_val,
                                                        dim_h0_amb_input_val,
                                                        matrix_pca_input_val],
                                                       [h11_labels_nn_val,
                                                        h21_labels_nn_val]
                                                      )
                                     )
    ...
    if path.isfile(path.join(MOD_PATH, 'cnn_functional_pca_dense.h5')):
    ...
    print('    Accuracy (rint) of the predictions for h_11: {:.3f}%'.format(\
                 accuracy_score(h11_labels_test,
                                model_cnn.predict([num_cp_input_test,
                                                   dim_cp_input_test,
                                                   dim_h0_amb_input_test,
                                                   matrix_pca_input_test])[0].\
                                                           reshape(-1,),
                                rounding=np.rint)*100))
    ...
    save_fig('cnn_functional_pca_dense_error_eng')
    # plt.show()
    plt.close(fig)
