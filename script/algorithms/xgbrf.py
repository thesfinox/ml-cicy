...
def compute(df_name, n_iter=30, rounding=np.floor, seed=42):
    ...
    xgbrf_h11.fit(df_matrix_train, h11_labels_train)
    gridcv_score(xgbrf_h11, rounding=rounding)
    prediction_score(xgbrf_h11,
                     df_matrix_test,
                     h11_labels_test,
    ...
    xgbrf_h21.fit(df_matrix_train, h21_labels_train)
    gridcv_score(xgbrf_h21, rounding=rounding)
    prediction_score(xgbrf_h21,
                     df_matrix_test,
                     h21_labels_test,
                     use_best_estimator=True,
                     rounding=rounding)
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          xgbrf_h11.best_estimator_.predict(df_matrix_test),
                          rounding=rounding),
    ...
    count_plot(plot,
               error_diff(h21_labels_test,
                          xgbrf_h21.best_estimator_.predict(df_matrix_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('xgbrf_error_matrix')
    # plt.show()
    plt.close(fig)
    ...
    xgbrf_h11.fit(df_num_cp_train, h11_labels_train)
    gridcv_score(xgbrf_h11, rounding=rounding)
    prediction_score(xgbrf_h11,
                     df_num_cp_test,
                     h11_labels_test,
    ...
    xgbrf_h21.fit(df_num_cp_train, h21_labels_train)
    gridcv_score(xgbrf_h21, rounding=rounding)
    prediction_score(xgbrf_h21,
                     df_num_cp_test,
                     h21_labels_test,
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          xgbrf_h11.best_estimator_.predict(df_num_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{11}$')
    count_plot(plot,
               error_diff(h21_labels_test,
                          xgbrf_h21.best_estimator_.predict(df_num_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('xgbrf_error_num_cp')
    # plt.show()
    plt.close(fig)
    ...
    xgbrf_h11.fit(df_dim_cp_train, h11_labels_train)
    gridcv_score(xgbrf_h11, rounding=rounding)
    prediction_score(xgbrf_h11,
                     df_dim_cp_test,
                     h11_labels_test,
    ...
    xgbrf_h21.fit(df_dim_cp_train, h21_labels_train)
    gridcv_score(xgbrf_h21, rounding=rounding)
    prediction_score(xgbrf_h21,
                     df_dim_cp_test,
                     h21_labels_test,
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          xgbrf_h11.best_estimator_.predict(df_dim_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{11}$')
    count_plot(plot,
               error_diff(h21_labels_test,
                          xgbrf_h21.best_estimator_.predict(df_dim_cp_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('xgbrf_error_dim_cp')
    # plt.show()
    plt.close(fig)
    ...
    xgbrf_h11.fit(df_eng_h11_train, h11_labels_train)
    gridcv_score(xgbrf_h11, rounding=rounding)
    prediction_score(xgbrf_h11,
                     df_eng_h11_test,
                     h11_labels_test,
    ...
    xgbrf_h21.fit(df_eng_h21_train, h21_labels_train)
    gridcv_score(xgbrf_h21, rounding=rounding)
    prediction_score(xgbrf_h21,
                     df_eng_h21_test,
                     h21_labels_test,
    ...
    count_plot(plot,
               error_diff(h11_labels_test,
                          xgbrf_h11.best_estimator_.predict(df_eng_h11_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{11}$')
    count_plot(plot,
               error_diff(h21_labels_test,
                          xgbrf_h21.best_estimator_.predict(df_eng_h21_test),
                          rounding=rounding),
               title='Error distribution on the test set',
               xlabel='Difference from real value',
               legend='$h_{21}$')

    save_fig('xgbrf_error_eng')
    # plt.show()
    plt.close(fig)

    # Saving the feature engineered models
    print('\nSaving models...')
    MOD_FILE = path.join(MOD_PATH, 'xgbrf_h11.joblib.xz')
    dump(xgbrf_h11.best_estimator_, MOD_FILE, compress=('xz',9))
    MOD_FILE = path.join(MOD_PATH, 'xgbrf_h21.joblib.xz')
    dump(xgbrf_h21.best_estimator_, MOD_FILE, compress=('xz',9))
    print('Models saved!')
